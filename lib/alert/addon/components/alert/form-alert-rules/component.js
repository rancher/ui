...
export default Component.extend({
  ...
  enableMonitoringRoute: computed('pageScope', function() {
    const { router: { _router: { _routerMicrolib: { currentRouteInfos: routeInfos } } } } = this;
    const { router: { currentRouteName } } = this;
    const myRoute = ( routeInfos || [] ).findBy('name', currentRouteName);

    if (!myRoute?.router?.activeTransition?.isTransition) {
