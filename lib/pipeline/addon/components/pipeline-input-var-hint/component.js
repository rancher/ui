...
export default Component.extend({
  ...
  positionStyle: function() {
    ...
    var positionX = get(this, 'positionX'), positionY = get(this, 'positionY');
  ...
  didRender() {
    ...
    this.$(document).on('click.hint', clickHiden)
      .on('scroll.hint', scrollPosition);
  ...
  actions: {
    clickHint(val) {
      ...
      if ( matchedIndex !== -1 && triggerInputEle ) {
        ...
        var newVal = value.slice(0, matchedIndex).concat(val)
          .concat(value.slice(cursorPosition, value.length));
