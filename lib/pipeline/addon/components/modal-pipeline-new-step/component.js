  ...
  switch (module.type) {
    ...
    case 'task':
      ...
      if (!config.isShell) {
        ...
        Ember.set(config, 'entrypoint', '');
        Ember.set(config, 'command', '');
      }
      break;
    case 'build':
...
export default Ember.Component.extend(ModalBase, {
  ...
  model: null,
  pipeline: Ember.computed.alias('modalService.modalOpts.pipeline'),
  routeState: Ember.computed.alias('modalService.modalOpts.routeState'),
  type: 'task',
  errors: null,
  state: {
    saveDisabled: false
  ...
  actions: {
    add: function(success) {
      ...
      model.set&&model.set('env', arryParameters);
      modalOpts.cb(model);
      ...
      if(model.sourceCodeConfig&&pipeline.stages[1].name === 'build' && pipeline.stages[1].steps.length === 0){
        ...
        let selected = environmentTypes.find(ele => ele.label === routeState.language);
