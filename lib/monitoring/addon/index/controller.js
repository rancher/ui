...
export default Controller.extend({
  ...
  dashboardLink: computed('app.environment', 'cluster.id', function() {
    if ( get(this, 'app.environment') === 'development' ) {
      return `https://localhost:8005/c/${ escape(this.cluster.id) }`;
    } else {
      return `/dashboard/c/${ escape(this.cluster.id) }`;
    }
  }),
});
