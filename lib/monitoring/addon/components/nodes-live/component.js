...
export default NodesReservation.extend(Metrics, {
    ...
    next(() => {
      ...
      if (used === 0 || used === undefined) {
        ...
        if (reservation > used) {
          setProperties(this, {
            maxCpuWheel:     'outer',
            cpuTotalPercent: Math.round(used / reservation * 100),
          })
        } else {
          setProperties(this, {
            maxCpuWheel:     'inner',
            cpuTotalPercent: Math.round(reservation / used * 100) || 100,
          })
        }
      }
    })
  ...
  memoryTotalPercentOb: observer('memoryUsedTotal', 'memoryTotalPercent.subtitle', function() {
    next(() => {
      ...
      if (used === 0 || used === undefined) {
        ...
        if (reservation > used) {
          setProperties(this, {
            maxMemoryWheel:     'outer',
            memoryTotalPercent: Math.round(used / reservation * 100),
          })
        } else {
          setProperties(this, {
            maxMemoryWheel:     'inner',
            memoryTotalPercent: Math.round(reservation / used * 100) || 100,
          })
        }
      }
    })
  ...
  cpuLive: computed('graphs.[]', function() {
    ...
    filtered.map((s) => out += s.points[1][0])
  ...
  clusterChange: on('init', observer('scope.currentCluster.id', function() {
    this.send('query');
  })),
