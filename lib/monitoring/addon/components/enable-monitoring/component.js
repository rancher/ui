...
import CatalogUpgrade from 'monitoring/mixins/catalog-upgrade';
...
export default Component.extend(ReservationCheck, CatalogUpgrade, {
  ...
  actions: {
    ...
    enable(cb) {
      ...
      resource.doAction(action, { answers }).then(() => {
        ...
        if ( action === 'editMonitoring' && get(this, 'upgrade') && get(this, 'upgradeAvailable') ) {
          this.send('upgrade')
        }
      }).catch((err) => {
        cb(false);
  ...
  saveDisabled: computed('insufficientPrometheusMemory', 'insufficientPrometheusCpu', 'insufficient', 'enabled', function() {
