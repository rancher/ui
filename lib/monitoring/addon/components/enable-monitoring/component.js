...
const PROMETHEUS_REMOTE_WRITE = 'prometheus.remoteWrite['

function isRemoteUrlKey(key){
  var regex = /prometheus\.remoteWrite\[\d+\].url/
...
export default Component.extend(ReservationCheck, CatalogUpgrade, {
  ...
  actions: {
    ...
    enable(cb) {
      ...
      Object.keys(answers).filter((key) => key.startsWith(PROMETHEUS_REMOTE_WRITE)).forEach((key) => {
        delete answers[key]
      });

      (this.remoteWrites || []).forEach((url, index) => {
        answers[`${ PROMETHEUS_REMOTE_WRITE }${ index }].url`] = url
      })
  ...
  updateConfig(answers) {
    ...
    this.updateRemoteWriteEndpoints(answers);
  },

  updateRemoteWriteEndpoints(answers) {
    const remoteWrites = [];

    Object.keys(answers).filter((key) => key.startsWith(PROMETHEUS_REMOTE_WRITE) ).forEach((key) => {
      remoteWrites.push(answers[key]);
    });

    set(this, 'remoteWrites', remoteWrites);
  }
});
