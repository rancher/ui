...
export default Component.extend(ReservationCheck, CatalogUpgrade, {
  ...
  clusterLevelMinCpu: computed('cluster.enableClusterMonitoring', 'level', 'project.enableProjectMonitoring', 'scope.currentCluster.nodes', function() {
  ...
  clusterLevelMinMemory: computed('cluster.enableClusterMonitoring', 'level', 'project.enableProjectMonitoring', 'scope.currentCluster.nodes', function() {
    const allNodes = get(this, 'scope.currentCluster.nodes') || [];
    const schedulableNodes = allNodes.filterBy('isUnschedulable', false);
  ...
  insufficientPrometheusCpu: computed('enabled', 'prometheusRequestCpu', 'requestsCpu', 'schedulableNodes.@each.{allocatable,requested}', 'scope.currentCluster.nodes.@each.{allocatable,requested}', function() {
    const allNodes = get(this, 'scope.currentCluster.nodes') || [];
  ...
  insufficientPrometheusMemory: computed('enabled', 'prometheusRequestMemory', 'requestsMemory', 'schedulableNodes.@each.{allocatable,requested}', 'scope.currentCluster.nodes.@each.{allocatable,requested}', function() {
    const allNodes = get(this, 'scope.currentCluster.nodes') || [];
