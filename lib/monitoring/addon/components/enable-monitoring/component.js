...
const EXPOSED_OPTIONS = ['exporter-node.enabled', 'exporter-node.ports.metrics.port',
  ...
  'grafana.persistence.storageClass', 'grafana.persistence.size', 'grafana.reportingEnabled',
...
export default Component.extend(ReservationCheck, CatalogUpgrade, {
  ...
  actions: {
    ...
    enable(cb) {
      ...
      answers['grafana.reportingEnabled'] = `${ get(this, 'enableGrafanaReporting') }`;
  ...
  updateConfig(answers) {
    ...
    if ( answers['grafana.reportingEnabled'] ) {
      set(this, 'enableGrafanaReporting', answers['grafana.reportingEnabled'] === 'true');
    }
