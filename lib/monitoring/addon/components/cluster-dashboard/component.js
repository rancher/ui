...
export default Component.extend(CatalogUpgrade, {
  ...
  hideManagerAndSchedulerStatus: computed('cluster.{clusterProvider,provider}', function() {
    const provider = get(this, 'cluster.clusterProvider');
    const hiddenClusterProviders = ['aks', 'tencent', 'lke']; // actual provider value set on cluster object, different then clusterProvider which is computed property and in these cases would be 'import'

    const existsInList = C.GRAY_OUT_SCHEDULER_STATUS_PROVIDERS.any((item) => {
      if (provider === item) {
        return true;
      } else if (provider === 'import' && hiddenClusterProviders.includes(get(this, 'cluster.provider'))) {
        return true;
      }
