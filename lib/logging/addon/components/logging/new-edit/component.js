...
export default Component.extend(NewOrEdit, {
  ...
  graylogWillSave(){
    const { model = {}, intl } = this
    const errors = []
    const config = get(model, 'graylog.config') || {}
    const {
      endpoint, protocol, enableTls, sslVerify
    } = config
    const graylogConfig = get(model, 'graylogConfig') || {}

    if (!endpoint) {
      errors.pushObject(intl.t('loggingPage.graylog.endpoint.required'))
    }

    if (protocol === 'udp') {
      setProperties(graylogConfig, {
        certificate:   null,
        clientKey:     null,
        clientCert:    null,
        sslVerify:     false,
    ...
    if (protocol === 'tcp') {
      if (!enableTls) {
        ...
        if (!sslVerify) {
          set(graylogConfig, 'certificate', null)
        }
      }
    }
