...
export default Component.extend(NewOrEdit, Parse, {
  scope:                   service(),
  settings:                service(),
  globalStore:             service(),
  intl:                    service(),
...
`
  ...
  initCustomContent() {
    if (get(this, 'originalModel.targetType') === 'customTarget') {
      ...
      this.syncForm(get(this, 'originalModel.customTargetConfig.content'))
  ...
  syncForm(file) {
    const { fileObj = {} } = this.parseValue(file || get(this, 'customContent'))
    const type = fileObj['@type']
    ...
    if (type === 'forward') {
      config = C.LOGGING_FILE_TO_FORM.FLUENTD
      model = get(this, 'model.fluentForwarder.config')
      const elements = (get(fileObj, 'elements') || []).filter((e) => e.elementName === 'server')
      ...
      const fluentServers = elements.map((e) => {
        ...
        if (get(attrs, 'host') && get(attrs, 'port')) {
          endpoint = `${ get(attrs, 'host') }:${ get(attrs, 'port') }`
        } else if (get(attrs, 'host')) {
      ...
      set(model, 'fluentServers', fluentServers)
    }

    if (type === 'elasticsearch') {
      config = C.LOGGING_FILE_TO_FORM.ELASTICSEARCH
      model = get(this, 'model.elasticsearch.config')
    } else if (type === 'splunk_hec') {
      config = C.LOGGING_FILE_TO_FORM.SPLUNK
      model = get(this, 'model.splunk.config')
    } else if (type === 'kafka_buffered') {
      config = C.LOGGING_FILE_TO_FORM.KAFKA
      model = get(this, 'model.kafka.config')
    } else if (type === 'remote_syslog') {
      config = C.LOGGING_FILE_TO_FORM.SYSLOG
      model = get(this, 'model.syslog.config')
    }

    Object.keys(config).map((key) => {
      if (key === 'host') {
        if (get(fileObj, 'host') && get(fileObj, 'port')) {
          set(model, 'endpoint', `${ get(fileObj, 'host') }:${ get(fileObj, 'port') }`)
        } else if (get(fileObj, 'host')) {
          set(model, 'endpoint', `${ get(fileObj, 'host') }`)
        }
      } else if (key === 'brokers') {
        const brokers = get(fileObj, 'brokers')

        set(model, 'brokerEndpoints', brokers ? brokers.split(',') : null)
      } else if (key === 'logstash_dateformat') {
        set(model, 'dateFormat', DATE_FORMAT[fileObj[key]])
      } else {
        set(model, get(config, key), fileObj[key])
      }
    })
  }
});
