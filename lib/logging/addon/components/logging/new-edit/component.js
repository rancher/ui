...
export default Component.extend(NewOrEdit, {
  ...
  willSave() {
    ...
    if (get(this, 'pasteOrUpload')) {
      ...
      if (targetType === 'cloudwatch') {
        const cloudwatchErrors = this.cloudwatchWillSave()

        errors.pushObjects(cloudwatchErrors)
      }
    }
  ...
  cloudwatchWillSave() {
    ...
    if (!get(model, 'cloudwatch.config.accessKeyID')) {
      errors.pushObject(intl.t('loggingPage.cloudwatch.accessKeyIDRequired'))
    }

    if (!get(model, 'cloudwatch.config.secretAccessKey')) {
      errors.pushObject(intl.t('loggingPage.cloudwatch.secretAccessKeyRequired'))
    }

    if (!get(model, 'cloudwatch.config.region')) {
      errors.pushObject(intl.t('loggingPage.cloudwatch.regionRequired'))
    }

    if (!get(model, 'cloudwatch.config.group')) {
      errors.pushObject(intl.t('loggingPage.cloudwatch.groupRequired'))
    }

    if (!get(model, 'cloudwatch.config.stream')) {
      errors.pushObject(intl.t('loggingPage.cloudwatch.streamRequired'))
    }
