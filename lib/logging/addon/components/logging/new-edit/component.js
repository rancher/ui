...
export default Component.extend(NewOrEdit, {
  ...
  willSave() {
    ...
    if (get(this, 'pasteOrUpload')) {
      set(model, 'customTargetConfig', {})
      set(model, 'customTargetConfig.content', get(this, 'customContent'))
      let fileObj = {}
      ...
      const types = ['elasticsearch', 'splunk_hec', 'remote_syslog', 'kafka_buffered', 'forward']
      ...
      setProperties(model, {
        outputFlushInterval:       get(model, `customTarget.outputFlushInterval`),
        outputTags:                get(model, `customTarget.outputTags`),
        dockerRoot:                get(model, `customTarget.dockerRoot`),
        excludeSystemComponent:    get(model, `customTarget.excludeSystemComponent`),
      })
    } else {
