...
export default Component.extend(ModalBase, {
  ...
  didReceiveAttrs() {
    ...
    return get(this, 'globalStore').findAll('nodeDriver').then((allNodeDrivers) => {
      ...
      allNodeDrivers.forEach((driver) => {
        if (!isEmpty(get(driver, 'uiUrl'))) {
          uiUrlPromises.pushObject(this.globalStore.find('schema', `${ get(driver, 'name') }config`));
        }
      });

      return allSettled(uiUrlPromises).then(() => {
