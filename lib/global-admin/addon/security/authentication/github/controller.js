...
import { denormalizeName } from 'ui/settings/service';
...
export default Controller.extend({
  ...
  numUsers: function() {
    return []; //TODO
    // return this.get('model.allowedPrincipalIds').filterBy('externalIdType',C.PROJECT.TYPE_GITHUB_USER).get('length');
  }.property('model.allowedPrincipalIds.@each.externalIdType','wasRestricted'),

  numOrgs: function() {
    return []; //TODO
    // return this.get('model.allowedPrincipalIds').filterBy('externalIdType',C.PROJECT.TYPE_GITHUB_ORG).get('length');
  }.property('model.allowedPrincipalIds.@each.externalIdType','wasRestricted'),
  ...
  actions: {
    save: function() {
      ...
      let githubConfig = this.get('githubConfig');
      ...
      this.get('model').doAction('configureTest', get(this, 'model')).then((resp) => {
        this.setProperties({
          saving: false,
          saved: true,
          haveToken: true,
        ...
        this.get('github').setProperties({
          redirectUrl: get(resp, 'redirectUrl')
        });


        //redirect? window.opener
    ...
    gotCode: function(code) {
      let ghConfig = get(this, 'githubConfig').clone();
      let out = {
        code: code,
        enabled: false,
      ...
      get(this, 'githubConfig').doAction('testAndApply', out).catch((err) => {
    ...
    authenticationSucceeded: function(auth) {
      ...
      model.setProperties({
        ...
        'allowedPrincipalIds': [auth.userIdentity],
      });
