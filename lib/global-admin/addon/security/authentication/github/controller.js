...
export default Controller.extend(AuthMixin, {
  ...
  numUsers: computed('authConfig.allowedPrincipalIds', 'authConfig.allowedPrincipals.@each.externalIdType', 'wasRestricted', function() {
    return ( get(this, 'authConfig.allowedPrincipalIds') || []).filter((principal) => principal.includes(C.PROJECT.TYPE_GITHUB_USER)).get('length');
  }),

  numOrgs: computed('authConfig.allowedPrincipalIds', 'authConfig.allowedPrincipals.@each.externalIdType', 'wasRestricted', function() {
