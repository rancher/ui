...
const HEADERS = [
  ...
  },
  {
    name:           'administrative',
    sort:           ['administrative'],
    translationKey: 'rolesPage.index.table.administrative',
    width:          140,
  },
  {
    name:           'Default',
...
export default Controller.extend({
  ...
  filteredContent: computed('context', 'model.roleTemplates.@each.{name,state,transitioning}', 'showOnlyDefaults', function() {
    ...
    case 'project':
      ...
      headers = headers.filter((header) => header.name !== 'administrative');
    ...
    case 'global':
      ...
      headers = headers.filter((header) => ['builtin', 'administrative'].indexOf(header.name) === -1 );
