...
export default Controller.extend(FilterState, {
  ...
  projectRows: computed('filtered.@each.{name,state}', function() {
    ...
    return get(this, 'model.roleTemplates').filter( (role ) => !get(role, 'hidden') && get(role, 'context') !== 'cluster');
  }),

  clusterRows: computed('filtered.@each.{name,state}', function() {

    return get(this, 'model.roleTemplates').filter( (role ) => !get(role, 'hidden') && get(role, 'context') !== 'project');
  }),
  ...
  setContent() {
    ...
    set(this, 'filterableContent', content);
