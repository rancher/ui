...
export default Mixin.create(NewOrEdit, ManageLabels, {
  ...
  willSave() {
    ...
    if ( get(this, 'primaryResource.type').toLowerCase() === 'nodetemplate') {
  ...
  doSave() {
    ...
    if ( type === 'nodetemplate' ) {
      return this._super(...arguments);
    } else if ( type === 'node'  && !passBack ) {
    ...
    if ( type === 'nodetemplate') {
      if (passBack) {
        tpl = get(this, 'globalStore').createRecord({
          type: 'nodeConfig',
          nodeTemplateId: get(this, 'model.id'),
      ...
      // There is no passBack for actual node
      return this._super(...arguments);
    } else if ( type === 'nodeconfig' && passBack ) {
      ...
      if (passBack) {
        tpl = get(this, 'globalStore').createRecord({
          type: 'nodeConfig',
          nodeTemplateId: get(this, 'primaryResource.nodeTemplateId'),
          requestedHostname: get(this, 'primaryResource.name'),
          displayName:  get(this, 'primaryResource.name'),
          state: 'to-create',
          role: [],
