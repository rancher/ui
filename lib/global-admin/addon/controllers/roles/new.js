...
export default Controller.extend(NewOrEdit, {
  ...
  initRoles: function () {
    this.set('roleArray', (this.get('primaryResource.roles') || []).map(r => {
      return {
        value: r
      };
    }));
  }.observes('primaryResource.role'),

  roleDidChange: function () {
    const role = this.get('model.role');
    role.set('roles', (this.get('roleArray') || []).filter(r => r.value).map(r => r.value));
  ...
  willSave() {
    ...
    if (ok) {
      ...
      const otherRoles = role.get('roles').uniq();
      role.set('name', name);
      role.set('roles', otherRoles);
    }
    return ok;
  },
