...
export default Route.extend({
    ...
    return this.globalStore.find('clustertemplate', params.template_id).then((template) => {
      return this.globalStore.findAll('clustertemplaterevision').then((revisions) => {
        ...
        if (template.defaultRevisionId) {
          ...
          if ( !isEmpty(tempRevision.questions)) {
            tempRevision.questions = this.checkKubernetesVersionOverride(tempRevision.questions);
          }
  ...
  checkKubernetesVersionOverride(questions = []) {
    let k8sVersion = questions.findBy('variable', 'rancherKubernetesEngineConfig.kubernetesVersion');

    if (k8sVersion && k8sVersion.hasOwnProperty('satisfies')) {
      questions = questions.filter((q) => {
        return isEmpty(q.satisfies);
      });
    }
