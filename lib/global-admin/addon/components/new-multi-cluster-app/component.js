...
export default Component.extend(NewOrEdit, CatalogApp, {
  ...
  updateAnswerOverrides: observer('selectedTemplateModel', function() {
    const questions       = get(this, 'selectedTemplateModel.questions')
      ...
      if (!isGlobalAnswersCollection(answer)) {
        Object.keys(answer.values).forEach((valueKey) => {
          ...
          let question = questions.find((q) => {
            if (q.variable === valueKey) {
              return q;
            } else if (q.subquestions.findBy('variable', valueKey)) {
              isSubQuestion = true;

              return q.subquestions.findBy('variable', valueKey);
            }
          });

          let nueOverride = {
            scope:         answer.clusterId || answer.projectId,
            answer:        JSON.parse(get(answer.values, valueKey)),
