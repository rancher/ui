...
export default Component.extend(NewOrEdit, CatalogApp, {
  ...
  answersArray: computed('primaryResource.answers.firstObject.values', 'selectedTemplateModel.{customAnswers,questions}', function() {
  ...
  answersString: computed('answersArray.@each.{answer,variable}', 'selectedTemplateModel', function() {
    const model = get(this, 'selectedTemplateModel');
  ...
  allProjectsAndClustersUngrouped: computed('clusters', 'primaryResource.targets.@each.projectId', 'projects.[]', function() {
    let out = [];
