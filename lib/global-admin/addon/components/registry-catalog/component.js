...
export default Component.extend(CrudCatalog, ReservationCheck, {
  ...
  registryWarning: computed('insufficientCpu', 'insufficientMemory', function() {
    let {
  ...
  insufficientDatabaseCpu: computed('databaseSchedulableNodes.@each.{allocatable,requested}', 'config.databaseRequestCpu', 'cluster.nodes.@each.{allocatable,requested}', function() {
    return this.getComponetInsufficient('database', 'cpu')
  }),
  ...
  insufficientDatabaseMemory: computed('databaseSchedulableNodes.@each.{allocatable,requested}', 'config.databaseRequestMemory', 'cluster.nodes.@each.{allocatable,requested}', function() {
    return this.getComponetInsufficient('database', 'memory')
  }),
  ...
  insufficientRedisCpu: computed('redisSchedulableNodes.@each.{allocatable,requested}', 'config.redisRequestCpu', 'cluster.nodes.@each.{allocatable,requested}', function() {
    return this.getComponetInsufficient('redis', 'cpu')
  }),
  ...
  insufficientRedisMemory: computed('redisSchedulableNodes.@each.{allocatable,requested}', 'config.redisRequestMemory', 'cluster.nodes.@each.{allocatable,requested}', function() {
    return this.getComponetInsufficient('redis', 'memory')
  }),
  ...
  pilotWarning: computed('insufficientPilotCpu', 'insufficientPilotMemory', 'pilotNodeSelectors.[]', function() {
    const {
      insufficientPilotCpu, insufficientPilotMemory, pilotNodeSelectors = [], intl
    } = this
    ...
    if (pilotNodeSelectors.length === 0) {
      prefix = 'globalRegistryPage.insufficientSize.workload'
    }
  ...
  leftMemory: computed('cluster.node.@each.{allocatable,requested}', function() {
    const allNodes = get(this, 'cluster.nodes') || [];
  ...
  saveDisabled: computed('registryWarning', 'databaseWarning', 'redisWarning', 'pilotWarning', function() {
    const {
      registryWarning, databaseWarning, redisWarning, pilotWarning
    } = this

    return databaseWarning || redisWarning || registryWarning || pilotWarning
  }),
