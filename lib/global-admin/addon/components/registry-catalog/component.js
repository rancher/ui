...
import { ucFirst, parseSi } from 'shared/utils/util';
...
export default Component.extend(CrudCatalog, ReservationCheck, {
  ...
  requestsCpu: computed('config.redisRequestCpu', 'config.databaseRequestCpu', function() {
    return ['redisRequestCpu', 'databaseRequestCpu'].reduce((all, key) => {
  ...
  requestsMemory: computed('config.redisRequestMemory', 'config.databaseRequestMemory', function() {
    return ['redisRequestMemory', 'databaseRequestMemory'].reduce((all, key) => {
      return all + parseInt(get(this, `config.${ key }`))
    }, 0)
  }),
  ...
  saveDisabled: computed('registryWarning', 'databaseWarning', 'redisWarning', function() {
    const {
      registryWarning, databaseWarning, redisWarning
    } = this

    return databaseWarning || redisWarning || registryWarning
  }),
