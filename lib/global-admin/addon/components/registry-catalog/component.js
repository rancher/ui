...
const REGISTRY_NODE_SELECTOR_PREFIX  = 'registry.registry.nodeSelector."'
...
export default Component.extend(CrudCatalog, ReservationCheck, {
      ...
      WORKLOADS.map((component) => {
        (get(this, `${ component }NodeSelectors`) || []).map((selector) => {
          ...
          if (component === 'registry') {
            answers[`registry.registry.nodeSelector."${ key }"`] = value
  ...
  requestsCpu: computed('config.redisRequestCpu', 'config.databaseRequestCpu', 'config.registryRequestCpu', 'config.notaryRequestCpu', 'config.notaryRequestMemory', function() {
    return WORKLOADS
      .filter((w) => {
      ...
      .reduce((all, w) => {
        return all + parseInt(get(this, `config.${ w }RequestCpu`))
      }, 0)
  }),
  ...
  requestsMemory: computed('config.redisRequestMemory', 'config.databaseRequestMemory', function() {
    return WORKLOADS
      ...
      .reduce((all, w) => {
        return all + parseInt(get(this, `config.${ w }RequestMemory`))
      }, 0)
  }),
  ...
  initConfig() {
    ...
    if ( get(this, 'enabled') ) {
      ...
      Object.keys(answers).filter((key) => key.startsWith(REGISTRY_NODE_SELECTOR_PREFIX) ).map((k) => {
        let value = answers[k] || '';
        const key = k.replace(REGISTRY_NODE_SELECTOR_PREFIX, '').slice(0, -1).replace(/\\\./g, '.')
      ...
      Object.keys(answers).filter((key) => key.startsWith(CLAIR_NODE_SELECTOR_PREFIX) ).map((k) => {
        let value = answers[k] || '';
        const key = k.replace(CLAIR_NODE_SELECTOR_PREFIX, '').slice(0, -1).replace(/\\\./g, '.')
      ...
      Object.keys(answers).filter((key) => key.startsWith(NOTARY_NODE_SELECTOR_PREFIX) ).map((k) => {
        let value = answers[k] || '';
        const key = k.replace(NOTARY_NODE_SELECTOR_PREFIX, '').slice(0, -1).replace(/\\\./g, '.')
      ...
      setProperties(this, {
        ...
        preRequestsCpu,
        preRequestsMemory,
        preRegistryRequestCpu:         convertToMillis(answers[REGISTRY_REQUEST_CPU]),
        preRegistryRequestMemory:      parseSi(answers[REGISTRY_REQUEST_MEM]),
        ...
        preClairRequestCpu:         convertToMillis(answers[CLAIR_REQUEST_CPU]),
        preClairRequestMemory:      parseSi(answers[CLAIR_REQUEST_MEM]),
        preNotaryRequestCpu:         convertToMillis(answers[NOTARY_REQUEST_CPU]),
        preNotaryRequestMemory:      parseSi(answers[NOTARY_REQUEST_MEM]),
      })
