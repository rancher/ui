...
// this route is never hit, if we ever introduce it again we should deal with these
...
export default Component.extend(CrudCatalog, ReservationCheck, {
  ...
  dockerLogin: computed('app.answers', 'showForm', function() {
  ...
  insufficientNotaryCpu: computed(`notarySchedulableNodes.@each.{allocatable,requested}`, `config.notaryServerRequestCpu`, 'cluster.nodes.@each.{allocatable,requested}', 'config.{notaryServerRequestCpu,notarySignerRequestCpu}', function() {
  ...
  insufficientNotaryMemory: computed(`notarySchedulableNodes.@each.{allocatable,requested}`, `config.notaryServerRequestMemory`, 'cluster.nodes.@each.{allocatable,requested}', 'config.{notaryServerRequestMemory,notarySignerRequestMemory}', function() {
  ...
  notaryWarning: computed(`insufficientNotaryCpu`, `insufficientNotaryMemory`, 'config.{notaryEnabled,notaryServerRequestCpu,notaryServerRequestMemory,notarySignerRequestCpu,notarySignerRequestMemory}', function() {
    if (!get(this, 'config.notaryEnabled')) {
      return
    }
