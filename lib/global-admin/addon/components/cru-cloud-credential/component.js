...
const CRED_CONFIG_CHOICES = [
  ...
  },
  {
    ...
    displayName: 'Packet',
    driver:      'packet',
    configField: 'packetcredentialConfig',
  },
  {
    name:        'vmware',
...
export default Component.extend(ViewNewEdit, {
  ...
  config: computed('cloudCredentialType', 'model.{amazonec2credentialConfig,azurecredentialConfig,digitaloceancredentialConfig,linodecredentialConfig,ocicredentialConfig,packetcredentialConfig,vmwarevspherecredentialConfig}', function() {
    const { model }   = this;
  ...
  willSave() {
    ...
    const { cloudCredentialType }      = this;
    const keysThatWeCanValidate = ['amazon', 'digitalOcean', 'linode', 'packet'];
    const auth                  = {
      type:  'validate',
      token: null,
    };
    ...
    if (keysThatWeCanValidate.includes(cloudCredentialType)) {
      ...
      if (cloudCredentialType === 'packet') {
        set(auth, 'token', get(this, 'config.token'));

        return this.packet.request(auth, 'user').then(() => {
          set(this, 'validatingKeys', false);
