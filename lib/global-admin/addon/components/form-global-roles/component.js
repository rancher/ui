...
export default Component.extend({
  ...
  init() {
    ...
    const hasCustom = !!get(user, 'globalRoleBindings').map((x) => get(x, 'globalRole'))
      .findBy('isCustom');
  ...
  save() {
    ...
    case CUSTOM:
      add    = custom.filterBy('active', true ).filterBy('existing', null)
        .map((x) => this.make(x.role));
      remove = custom.filterBy('active', false).filterBy('existing')
        .map((y) => y.existing);
