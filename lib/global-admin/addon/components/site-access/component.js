...
import { get, set, observer, computed, setProperties } from '@ember/object';
...
export default Component.extend({
  ...
  accessModeChanged: on('init', observer('model.accessMode', function() {
    ...
    if ( get(this, 'model.accessMode') !== 'unrestricted' ) {
      ...
      const me    = get(this, 'access.principal')||[];
      const found = allowedPrincipals.filter(ident => get(me, 'id') === ident).length > 0;
  ...
  checkDuplicate(id) {
    return get(this, 'principals').includes(id) ? true : false;
  },
