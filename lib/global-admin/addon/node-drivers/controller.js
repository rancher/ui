...
export default Controller.extend({
  ...
  actions: {
    ...
    addCatalogDriver(driver) {
      ...
      this.get('globalStore').request({ url: `${ this.get('app.catalogEndpoint') }/templates/${ driver.id }` })
        .then((template) => {

          this.get('globalStore').request({ url: template.versionLinks[template.defaultVersion] })
            .then((driver) => {

              let newDriver = this.createNewDriver(driver);

              this.get('globalStore').createRecord(newDriver)
                .save()
                .catch((err) => {
    ...
    upgradeDriver(driver, version, url) {
      ...
      this.get('globalStore').request({ url })
        .then((template) => {
