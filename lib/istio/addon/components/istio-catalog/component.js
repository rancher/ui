...
export default Component.extend(CrudCatalog, ReservationCheck, CatalogUpgrade, {
  ...
  grafanaUrl: computed('cluster.id', function() {
    return `/k8s/clusters/${ get(this, 'cluster.id') }/api/v1/namespaces/cluster-monitoring/services/http:grafana:80/proxy/`
  }),

  prometheusUrl: computed('cluster.id', function() {
    return `/k8s/clusters/${ get(this, 'cluster.id') }/api/v1/namespaces/cluster-monitoring/services/http:prometheus-http:80/proxy/`
  }),
  ...
  canEnable: computed('scope.currentCluster.enableClusterMonitoring', function() {
