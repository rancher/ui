...
export default Component.extend(CrudCatalog, ReservationCheck, {
  ...
  requestsCpu: computed('config.mixerRequestCpu', 'config.prometheusRequestCpu', 'config.pilotRequestCpu', 'config.gatewayRequestCpu', 'grafanaRequestCpu', 'tracingRequestCpu', function() {
    return WORKLOADS
      .filter((w) => {
  ...
  initAnswers() {
    ...
    Object.keys(answers).filter((key) => key.startsWith(GRAFANA_NODE_SELECTOR_PREFIX) ).map((k) => {
      let value = answers[k] || '';
      const key = k.replace(GRAFANA_NODE_SELECTOR_PREFIX, '').slice(0, -1).replace(/\\\./g, '.')
    ...
    Object.keys(answers).filter((key) => key.startsWith(GATEWAY_NODE_SELECTOR_PREFIX) ).map((k) => {
      let value = answers[k] || '';
      const key = k.replace(GATEWAY_NODE_SELECTOR_PREFIX, '').slice(0, -1).replace(/\\\./g, '.')
    ...
    Object.keys(answers).filter((key) => key.startsWith(TRACING_NODE_SELECTOR_PREFIX) ).map((k) => {
      let value = answers[k] || '';
      const key = k.replace(TRACING_NODE_SELECTOR_PREFIX, '').slice(0, -1).replace(/\\\./g, '.')
    ...
    setProperties(this, {
      ...
      preGatewayRequestCpu:       convertToMillis(answers[GATEWAY_REQUEST_CPU]),
      preGatewayRequestMemory:    parseSi(answers[GATEWAY_REQUEST_MEM]),
      preTracingRequestCpu:       convertToMillis(answers[TRACING_REQUEST_CPU]),
      preTracingRequestMemory:    parseSi(answers[TRACING_REQUEST_MEM]),
    })
