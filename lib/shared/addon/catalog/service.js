...
export default Service.extend({
  ...
  templateBase: computed('scope.current.orchestration', function() {
    return this.get('scope.current.orchestration') || '';
  }),
  ...
  refresh() {
    ...
    return this.fetchCatalogs().then(() => {
      ...
      return store.request({
        ...
        headers: {[C.HEADER.PROJECT_ID]: this.get('scope.current.id')},
        method: 'POST',
  ...
  fetchCatalogs(opts) {
    var neu = $.extend({headers: {[C.HEADER.PROJECT_ID]: this.get('scope.current.id')}}, {url: `${this.get('app.catalogEndpoint')}/catalogs`}, opts||{});
  ...
  fetchTemplates(params) {
    ...
    return this.get('store').request({url: url, headers: {[C.HEADER.PROJECT_ID]: this.get('scope.current.id')}}).then((res) => {
  ...
  fetchByUrl(url) {
    return this.get('store').request({url: url, headers: {[C.HEADER.PROJECT_ID]: this.get('scope.current.id')}});
  },
