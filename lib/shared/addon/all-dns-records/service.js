...
export default Service.extend({
  ...
  init() {
    ...
    set(this, '_allDnsRecords', store.all('dnsRecord'));
  ...
  dnsRecords: function() {
    ...
    return get(this, '_allDnsRecords').filter((x) => get(x, 'ownerReferences.firstObject.kind') !== 'Ingress')
      .map((x) => convert(x, intl))
      .sortBy('combined');
  ...
  group(list) {
    ...
    list.slice().sortBy('group', 'name', 'id')
      .forEach((obj) => {
