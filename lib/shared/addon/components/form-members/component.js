...
export default Component.extend({
          ...
          bindings.forEach((memberBinding) => {
            if (!preEditBindings.findBy('roleTemplateId', memberBinding)) {
              if (get(member, 'principalId')) {
                ...
                if (type === 'group') {
                  add.push(this.createRole({groupPrincipalId: principalId, roleTemplateId: memberBinding, subjectKind: 'Group'}));
                } else {
                  add.push(this.createRole({userPrincipalId: principalId, roleTemplateId: memberBinding, subjectKind: 'User'}));
                }
              }
            }
          });
  ...
  hasOwner: function(toRemove, toAdd) {

    let okay = true;

    let match = toRemove.find((member) => {
      return get(member, 'roleTemplateId').indexOf('owner') >= 0;
    })
    if (match) {
      if (get(toAdd, 'length') >= 0) {
        let match = toAdd.find((member) => {
          return get(member, 'roleTemplateId').indexOf('owner') >= 0;
        })
        if (!match) {
          okay=false;
