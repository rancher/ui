...
export default Component.extend({
  ...
  primaryResourceSaved: function() {
    ...
    const fromCustomToNon = memberArray.filterBy('fromCustomToNon');
    ...
    fromCustomToNon.forEach((m) => {
      get(m, 'fromCustomToNon').forEach((a) => {
        let role = get(m, 'role').clone();
        set(role, 'roleTemplateId', a);
        add.addObject(role);
      })
