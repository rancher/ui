    ...
    customToRemove.forEach((rm) => {
      ...
      get(rm, 'customRolesToRemove').forEach((roleTempId) => {
        let match = get(this, '_bindings').find((binding) => {
          if ((get(binding, 'roleTemplateId') === roleTempId) || ( get(binding, 'roleTemplateId') === 'custom' && get(rm, '_roleTemplateId') )) {
            let out = binding;
            if ( get(binding, 'roleTemplateId') === 'custom' && get(rm, '_roleTemplateId') ) {
              var config = {
                type: get(binding, 'type'),
                subjectName: get(binding, 'subjectName'),
                subjectKind: get(binding, 'subjectKind'),
                roleTemplateId: get(rm, '_roleTemplateId'),
              };

              if (get(this, 'type') === 'cluster') {
                config['clusterId'] = get(binding, 'clusterId');
              } else {
                config['projectId'] = get(binding, 'projectId');
              }
              out = get(this, 'globalStore').createRecord(config);
            }
            return out;
          }
        });
        // TODO MATCH IS ALWAYS RETURNING THE OLD ROLE WITH CUSTOM I HAVE NO IDEA WHY
        remove.push(match);
      });
    });
