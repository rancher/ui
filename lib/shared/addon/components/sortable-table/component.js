...
export default Component.extend(Sortable, StickyHeader, {
  ...
  actionsChanged: observer('selectedNodes.@each._availableActions','pagedContent.firstObject._availableActions', function() {
    ...
    if (nodes.length >= 1) {
      ...
      out = nodes.get('firstObject._availableActions').filterBy('bulkable',true).map((act) => {
      ...
      nodes.forEach((node) => {
        let bad;
        if ( disableAll ) {
          bad = get(node,'_availableActions').map(act => act.action);
        } else {
          bad = get(node,'_availableActions').filterBy('enabled',false).map(act => act.action);
        }
