...
export default Component.extend(ModalBase, {
  ...
  filteredRoles: computed('modalOpts.roles', function() {
    ...
    return get(this, 'modalOpts.roles').filterBy('isCustom')
      .map((role) => {
  ...
  actions: {
    save(cb) {
      ...
      let customroles = get(this, 'filteredRoles').filter((c) => get(c, 'active'))
        .map((r) => get(r, 'role.id'));
      ...
      if (customroles.length > 0) {
        ...
        set(this, 'errors', ['You must choose a role for this memeber before closing the modal.'])
