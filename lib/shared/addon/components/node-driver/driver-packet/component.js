...
export default Component.extend(NodeDriver, {
  ...
  actions: {
    authPacket(savedCB) {
      ...
      hash(promises).then((hash) => {
        ...
        let config        = get(this,'packetConfig');
  ...
  apiRequest(command, opt, out) {
    ...
    return fetch(url, {
      headers: {
        ...
        'X-Auth-Token': get(this,'packetConfig.apiKey'),
      },
    }).then((res) => {
      let body = res.body;
  ...
  planChoiceDetails: computed('packetConfig.plan', function() {
    let planSlug = get(this,'packetConfig.plan');
  ...
  facilityObserver: on('init', observer('packetConfig.facility', function() {

    let facilities = get(this,'facilityChoices');
    let slug       = get(this,'packetConfig.facility');
  ...
  validate: function() {
    ...
    if (!get(this,'packetConfig.projectId') ) {
      errors.push('Project ID is required');
    }

    if (!get(this,'packetConfig.apiKey') ) {
