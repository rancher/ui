...
export default Component.extend({
  ...
  openModal() {
    ...
    if (!get(member, '_roleTemplateId')) {
      ...
      let neu = null;
      if (get(this, 'member.role.id') && get(this, 'member.role.roleTemplateId') === 'custom' && get(this, 'member.toUpdate')) {
        let idsOut = [];
        let role = get(member, 'role');

        idsOut.push(get(member, '_roleTemplateId'));

        var config = {
          type: get(role, 'type'),
          subjectName: get(role, 'subjectName'),
          subjectKind: get(role, 'subjectKind'),
          roleTemplateId: get(role, 'roleTemplateId'),
        };

        if (get(this, 'pageType') === 'cluster') {
          config['clusterId'] = get(role, 'clusterId');
        } else {
          config['projectId'] = get(role, 'projectId');
        }

        neu = get(this, 'globalStore').createRecord(config);
