...
export default Component.extend(Driver, {
  ...
  init() {
    ...
    globalStore.findAll('machine').then((all) => {
      ...
      this.notifyPropertyChange('initialMachineCounts');
    });
  ...
  command: computed(`settings.${C.SETTING.AGENT_IMAGE}`, 'labels', 'token.token', 'model.role.[]', function() {
