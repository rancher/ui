...
export default Component.extend(Driver, {
  ...
  actions: {
    ...
    exoscaleLogin: function() {
      ...
      this.set('exoscaleConfig.exoscaleApiSecretKey', (this.get('exoscaleConfig.exoscaleApiSecretKey')||'').trim());
    ...
    return ajaxPromise({url: url,
                        ...
                        beforeSend: (xhr, settings) => {
                          ...
                          settings.data += '&signature=' + encodeURIComponent(AWS.util.crypto.hmac(
                            this.get('exoscaleConfig.exoscaleApiSecretKey'), qs, 'base64', 'sha1'));
                          return true;
                        },
                        data: params}, true);
  },
