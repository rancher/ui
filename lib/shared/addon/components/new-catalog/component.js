...
export default Component.extend(NewOrEdit, {
  ...
  answersArray: computed('selectedTemplateModel.questions', 'catalogApp.answers', function() {
    if (get(this, 'templateKind') === 'helm') {
      return get(this, 'catalogApp.answers');
    } else {
      return get(this, 'selectedTemplateModel.questions');
    }
  }),
  ...
  didSave(neu) {
    ...
    if (get(this, 'upgrade')) {
      return app.doAction('upgrade', {externalId: get(this, 'upgrade.externalId')}).then(resp => {
        return resp;
      }).catch(err => {
        return err;
      });
