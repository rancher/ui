...
export default Component.extend(ClusterDriver, {
  ...
  useIpAliasesChange: observer('config.useIpAliases', function() {
    if (!get(this, 'config.useIpAliases')) {
      set(this, 'config.enablePrivateEndpoint', false)
    }
  }),
  ...
  willSave() {
    ...
    const clone = get(this, 'globalStore').createRecord({ type: 'googleKubernetesEngineConfig' })
    ...
    } else if (!get(config, 'ipPolicyCreateSubnetwork')) {
      setProperties(config, {
        ipPolicyClusterIpv4CidrBlock:  null,
        ipPolicyServicesIpv4CidrBlock: null,
      })
