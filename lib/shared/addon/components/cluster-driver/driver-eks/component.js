...
import { INSTANCE_TYPES, nameFromResource, EKS_REGIONS, EKS_VERSIONS } from 'shared/utils/amazon';
import {
  get, set, setProperties, computed, observer
} from '@ember/object';
import { Promise, resolve, reject } from 'rsvp';
import { alias, equal, union, not } from '@ember/object/computed';
...
export default Component.extend(ClusterDriver, {
  ...
  cloudCredentials:                   alias('model.cloudCredentials'),
  ...
  actions: {
    ...
    finishAndSelectCloudCredential(cred) {
      if (cred) {
        set(this, 'primaryResource.amazonCredentialSecret', get(cred, 'id'));
  ...
  selectedSubnets: computed('config.subnets.[]', 'primaryResource.eksStatus.generatedSubnets.[]', function() {
    const {
      config: { subnets },
      primaryResource: { eksStatus: { generatedSubnets } = { } },
    } = this;
