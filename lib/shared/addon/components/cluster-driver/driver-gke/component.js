...
export default Component.extend(ClusterDriver, {
  ...
  monitoringServiceChanged: observer('config.monitoringService', 'editing', function() {
    ...
    if (monitoringService) {
      if (monitoringService === 'none' && loggingService !== 'none' || !loggingService) {
        // set(this, 'config.loggingService', 'none');
        set(this, 'showLogMonServiceWarning', true);
      } else if (monitoringService === 'monitoring.googleapis.com/kubernetes' && loggingService === 'none') {
        // set(this, 'config.loggingService', 'logging.googleapis.com/kubernetes');
        set(this, 'showLogMonServiceWarning', true);
      } else {
        set(this, 'showLogMonServiceWarning', false);
      }
  ...
  versionChoices: computed('versions.validMasterVersions.[]', 'editing', function() {
    const {
    ...
    const masterSem = Semver.parse(initialVersion, { includePrerelease: true });
    let masterRange = null;

    if (this.editing && !isEmpty(masterSem?.minor)) {
      const neuMinor = masterSem.minor + 1;  // gke can only support 1 minor version bump

      masterRange =  `<=${ masterSem.major }.${ neuMinor }.x`;
    }

    return this.serviceVersions.parseCloudProviderVersionChoicesV2(validMasterVersions.slice(), initialVersion, mode, masterRange);
  }),
