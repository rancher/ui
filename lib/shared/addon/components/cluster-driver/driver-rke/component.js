...
export default Component.extend(ClusterDriver, {
  ...
  command: computed('labels', 'token.nodeCommand', 'etcd', 'controlplane', 'worker', 'address', 'internalAddress', function() {
    ...
    Object.keys(labels).forEach((key) => {
      labelCmdString += ` --label ${key}=${labels[key]}`;
    });

    if ( labelCmdString ) {
      const index = out.indexOf(DOCKER_RUN);

      if ( index > -1 ) {
        out = `${out.slice(0, index + DOCKER_RUN.length)}${labelCmdString}${out.slice(index + DOCKER_RUN.length)}`;
      }
    }
