...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  actions: {
    ...
    addRegistry(pr) {
      ...
      if (( config.privateRegistries || [] ).length <= 0) {
        ...
        get(this, 'privateRegistries').pushObject(pr);
      }
    },
  ...
  windowsSupportAvailable: computed('cluster.rancherKubernetesEngineConfig.network.plugin', 'cluster.rancherKubernetesEngineConfig.kubernetesVersion', function() {
