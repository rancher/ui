...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  init() {
    ...
    const maxUnavailable = get(this, 'primaryResource.rancherKubernetesEngineConfig.upgradeStrategy.maxUnavailable') || '';
    const maxUnavailableUnit = maxUnavailable.includes('%') ? 'percentage' : 'count';
    const drainInput = get(this, 'primaryResource.rancherKubernetesEngineConfig.upgradeStrategy.nodeDrainInput');

    set(this, 'upgradeStrategy.maxUnavailable', maxUnavailable.replace('%', ''));
    ...
    set(this, 'upgradeStrategy.drain', !!get(this, 'primaryResource.rancherKubernetesEngineConfig.upgradeStrategy.nodeDrainInput'));
  },
  ...
  maxUnavailable: computed('upgradeStrategy.maxUnavailableUnit', 'upgradeStrategy.maxUnavailable', function() {
    const value = { value: get(this, 'upgradeStrategy.maxUnavailable') };
  ...
  filteredTemplateRevisions: computed('selectedClusterTemplateId', 'model.clusterTemplateRevisions.@each.{id,state,name,members}', function() {
    ...
    let mapped = clusterTemplateRevisions.map((clusterTemplateRevision) => {
      ...
      const isDefault = clusterTemplate.defaultRevisionId === clusterTemplateRevision.id;
  ...
  willSave() {
    ...
    const upgradeStrategy = { ...get(this, 'upgradeStrategy') };

    drainNodeWillSave(upgradeStrategy.nodeDrainInput);

    if (upgradeStrategy.maxUnavailableUnit === 'percentage') {
      upgradeStrategy.maxUnavailable += '%';
    }

    if (!upgradeStrategy.drain) {
      upgradeStrategy.nodeDrainInput = null;
    }
    set(cluster, 'rancherKubernetesEngineConfig.upgradeStrategy', upgradeStrategy);
    ...
    // ensures that everything from the udpateFromYaml action has been applied to the config so validation will complete correctly
    next(() => {
      ok = this.validate();
    });
