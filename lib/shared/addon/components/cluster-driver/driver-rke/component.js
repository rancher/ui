...
export default InputTextFile.extend(ClusterDriver, {
  ...
  validateEtcdService(errors) {
    ...
    function hasUnit(value, unit) {
      return value.indexOf(unit) !== -1 ? true : false;
    }

    function parseDurationDetails(value, unit) {
      let pattern = /((\d+)[hms])/;
      let nue = value.split(unit).without('');
      let isValid = !isNaN(parseInt(nue[0])) && nue.length > 1;  // ===1 means no trailing h,m,s
      let remainder = [];
    ...
    function checkDuration(duration, type) {
      // duration = 12h12m12s || 12h12m || 12h  || 12m12s || 12m || 12s
      if (duration && !hasUnit(duration, 'h')) {
        if (!hasUnit(duration, 'm')) {
          seconds = parseDurationDetails(duration, 's');
          if (!seconds.isValid && !hasUnit(duration, 's')) {
            durationError(duration, type);
          }
        } else  { // no hours has min check rest
          minutes = parseDurationDetails(duration, 'm');
          seconds = minutes.remainder;

          if (!seconds.isValid && !hasUnit(seconds, 's')) {
            durationError(duration, type);
          }
        }
      } else {
        hours     = parseDurationDetails(duration, 'h')
        remainder = hours.remainder;

        if (hours.length > 1 || !hours.isValid ) {
          durationError(duration, type);
        } else if (remainder.length >= 1) {
          minutes   = parseDurationDetails(remainder, 'm');
          remainder = minutes.remainder;

          if (minutes.length > 1 || !minutes.isValid ) {
            durationError(duration, type);
          } else if (remainder.length > 1) {
            seconds = parseDurationDetails(remainder, 's');
            remainder = seconds.remainder;

            if (!seconds.isValid || remainder.length > 0) {
              durationError(duration, type);
            }
          }
        }
    ...
    checkDuration(creation, 'Creation');
    checkDuration(retention, 'Reteintion');
