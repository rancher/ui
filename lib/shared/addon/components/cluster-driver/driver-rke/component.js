...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  command: computed('taints', 'labels', 'token.nodeCommand', 'token.windowsNodeCommand', 'etcd', 'controlplane', 'worker', 'address', 'internalAddress', 'nodeName', 'isLinux', 'privateRegistryAddress', function() {
    ...
    const privateRegistryAddress = get(this, 'privateRegistryAddress')
    ...
    if (privateRegistryAddress) {
      const imageName = 'rancher/rancher-agent'

      if (windowsSelected) {
        out = out.replace(imageName, `-e AGENT_IMAGE=${ privateRegistryAddress }/${ imageName }`)
      } else {
        out = out.replace(imageName, `${ privateRegistryAddress }/${ imageName }`)
      }
    }
