...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  init() {
    ...
    scheduleOnce('afterRender', () => {
      ...
      if ( get(this, 'isEdit') && get(this, 'cluster.isWindows') ) {
  ...
  actions: {
    setCpFields(configName, cloudProviderConfig) {
      set(this, `cluster.rancherKubernetesEngineConfig.cloudProvider.${ configName }`, cloudProviderConfig);
    },
    ...
    removeRegistry(pr) {
      get(this, 'cluster.rancherKubernetesEngineConfig.privateRegistries').removeObject(pr);
    },
  },
  ...
  windowsSupportAvailable: computed('cluster.rancherKubernetesEngineConfig.network.plugin', 'config.kubernetesVersion', function() {
    const plugin = get(this, 'cluster.rancherKubernetesEngineConfig.network.plugin');
