...
import { underlineToCamel, removeEmpty, keysToCamel, validateEndpoint, keysToDecamelize } from 'shared/utils/util';
...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  actions: {
    yamlValUpdated(yamlValue, codeMirror) {
      set(this, 'codeMirrorInstance', codeMirror);
      set(this, 'yamlValue', yamlValue);
    },
    ...
    addOverride(enabled, paramsToOverride, hideQuestion = false) {
      ...
      if (enabled) {
        if (!clusterTemplateQuestions.findBy('variable', path)) {
          ...
          } else if (path.startsWith('uiOverride')) {
        ...
        } else if (path === 'uiOverrideBackupStrategy') {
  ...
  yamlValue: computed('pasteOrUpload', {
    get() {
      ...
      let config     = this.getClusterFields(this.primaryResource);
      const cpConfig = get(config, 'rancher_kubernetes_engine_config.cloud_provider');

      if ( cpConfig && cpConfig.azureCloudProvider ) {
    ...
    set(key, value) {
      let configs, parsedYaml;

      try {
        configs = jsyaml.safeLoad(value);
      } catch ( err ) {
      ...
      const rkeFields            = this.getResourceFields('rancherKubernetesEngineConfig');
      const clusterFields        = this.getResourceFields('cluster');
      const questionsSchemas     = this.clusterTemplates && (this.clusterTemplates.questionsSchemas || []).length > 0 ? this.clusterTemplates.questionsSchemas : [];
      const clusterKeys          = [];
      const { unknownKeys = [] } = this;
      const { codeMirrorInstance = null } = this;

      Object.keys(clusterFields).filter((key) => clusterFields[key].update).forEach((key) => {
        clusterKeys.push(camelToUnderline(key));
      });

      Object.keys(configs || {}).forEach((key) => {
        let camelizedKey = underlineToCamel(key);
        let configObj    = configs[key];

        if ( rkeFields[camelizedKey] || clusterFields[camelizedKey] ) {
          // its defined on the cluster or rke schemas
          if ( isEmpty(configObj) ) {
            return
          }

          let obj = keysToCamel(configObj);

          if (questionsSchemas && questionsSchemas.find((q) => q.variable.includes(camelizedKey))) {
            if (typeof configObj === 'object') {
              ...
              while (n--) {
                let path = `${ camelizedKey }.${ keys[n] }`;
                if (!questionsSchemas.find((q) => q.variable.includes(path))) {
                  if (!unknownKeys.findBy('keyName', path)) {
                    if (this.ingornedRkeOverrides[path]) {
                      // has UI component
                      unknownKeys.push({
                        keyName: path,
                        yamlKey: decamelize(path),
                        unknown: false,
                        info: codeMirrorInstance.doc.getCursor()
                      });
                    } else {
                      unknownKeys.push({
                        keyName: path,
                  ...
                  // console.log(keys[n], configObj, camelizedKey, path);
                }
              }
            }
          } else {
            // completely unknown
            if (!unknownKeys.findBy('keyName', camelizedKey)) {
              if (this.ingornedRkeOverrides[camelizedKey]) {
                // has UI component
                unknownKeys.push({
                  keyName: camelizedKey,
                  yamlKey: decamelize(camelizedKey),
                  unknown: false,
                  info: codeMirrorInstance.doc.getCursor()
                });
              } else {
          ...
          if ( key === 'rancher_kubernetes_engine_config.network' ) {
            obj = keysToCamel(configObj, ['options']);
          }

          if ( clusterKeys.includes(key) ) {
            set(this, `primaryResource.${ camelizedKey }`, obj)

            return
          }

          if ( key === 'rancher_kubernetes_engine_config.services' && obj['kube-api'] ) {
            set(obj, 'kubeApi', obj['kube-api']);

            delete obj['kube-api'];
          }

          if ( key === 'rancher_kubernetes_engine_config.services' && obj['kube-controller'] ) {
            set(obj, 'kubeController', obj['kube-controller']);

            delete obj['kube-controller'];
          }

          console.log(camelizedKey);

          set(this, `config.${ camelizedKey }`, obj);
        } else {
          // not defined on the cluster or rke fields
          if (!unknownKeys.findBy('keyName', camelizedKey)) {
            if (this.ingornedRkeOverrides[camelizedKey]) {
              // has UI component - we should never get here afaik
              unknownKeys.push({
                keyName: camelizedKey,
                yamlKey: key,
                unknown: false,
          ...
          console.log('unknown key', camelizedKey, configObj, this.primaryResource);
          set(this, `primaryResource.${ camelizedKey }`, configObj || null);
        }
      });

      // console.log(unknownKeys/* , this.primaryResource, this.config */);
      set(this, 'unknownKeys', unknownKeys);
