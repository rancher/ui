...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  actions: {
    addOverride(enabled, paramsToOverride) {
      let {
        primaryResource,
      ...
      if (clusterTemplateRevision && clusterTemplateRevision.questionsSchemas && ( clusterTemplateRevision.questionsSchemas.questions || []).length > 0) {
        questionsSchemas = clusterTemplateRevision.questionsSchemas.questions;
      }


      if (enabled) {
        if (path === 'windowsSupport') {
          clusterTemplateQuestions.pushObjects(this.populateWindowsSupportOverrides(clusterTemplateQuestions, questionsSchemas));
          ...
          set(question, 'isBuiltIn', true);

          defineProperty(question, 'default', alias(`primaryResource.${ path }`, () => {
            return get(primaryResource, path);
          }));
  ...
  populateWindowsSupportOverrides(overrides, questionsSchemas = []) {
