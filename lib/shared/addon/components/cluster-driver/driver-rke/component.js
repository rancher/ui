...
export default InputTextFile.extend(ManageLabels, ClusterDriver, {
  ...
  willSave() {
    ...
    if (upgradeStrategy.maxUnavailableUnit === 'percentage') {
      upgradeStrategy.maxUnavailableWorker += '%';
    }
    ...
    set(cluster, 'scheduledClusterScan', scheduledClusterScan);
  ...
  createRkeConfigWithDefaults() {
    ...
    const rkeConfig = globalStore.createRecord({
      ...
      upgradeStrategy: globalStore.createRecord({
        type:           'nodeUpgradeStrategy',
        nodeDrainInput: globalStore.createRecord({ type: 'nodeDrainInput' })
      })
    });
