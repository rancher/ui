...
export default Component.extend(ClusterDriver, {
  ...
  versionChoices: computed('versions', function() {
    ...
    return this.versionChoiceService.parseCloudProviderVersionChoices(( versions || [] ).reverse(), initialVersion, mode, null, false, MINIMUM_VERSION);
  }),
  ...
  validate() {
    ...
    const nodeResourceGroup = get(this, 'config.nodeResourceGroup')

    if (nodeResourceGroup){
      clusterErrors.push(...this.validateResourceGroup(nodeResourceGroup, intl.t('clusterNew.azureaks.nodeResourceGroup.label')))
    }

    const resourceGroup = get(this, 'config.resourceGroup')

    if (resourceGroup){
      clusterErrors.push(...this.validateResourceGroup(resourceGroup, intl.t('clusterNew.azureaks.resourceGroup.label')))
    }
  ...
  validateResourceGroup(resourceGroup, key){
    const intl   = get(this, 'intl');

    const errors = []

    if ( resourceGroup?.length > 80) {
      errors.push(intl.t('clusterNew.azureaks.errors.nodeResourceGroup.length', { key }))
    }
    ...
    if (!resourceGroup.match(/^([A-Z]|[a-z]|\p{Lu}|\p{Ll}|\p{Lt}|\p{Lo}|\p{Lm}|\p{Nd}|\.|-|_|\(|\))+$/u)) {
      errors.push(intl.t('clusterNew.azureaks.errors.nodeResourceGroup.chars', { key }))
    }
    if (resourceGroup.endsWith('.')){
      errors.push(intl.t('clusterNew.azureaks.errors.nodeResourceGroup.periodEnd', { key }))
    }
