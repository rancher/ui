...
export default Component.extend(ClusterDriver, {
  ...
  validate() {
    ...
    if (nodeResourceGroup){
      if ( nodeResourceGroup?.length > 80) {
        clusterErrors.push(intl.t('clusterNew.azureaks.errors.nodeResourceGroup.length'))
      }
      ...
      if (!nodeResourceGroup.match(/([A-Z]|[a-z]|\p{Lu}|\p{Ll}|\p{Lt}|\p{Lo}|\p{Lm}|\p{Nd}|\.|-|_|\(|\))/u)) {
        clusterErrors.push(intl.t('clusterNew.azureaks.errors.nodeResourceGroup.chars'))
      }
      if (nodeResourceGroup.endsWith('.')){
        clusterErrors.push(intl.t('clusterNew.azureaks.errors.nodeResourceGroup.periodEnd'))
      }
    }
    debugger
