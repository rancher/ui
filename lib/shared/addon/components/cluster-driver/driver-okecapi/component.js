...
export default Component.extend(ClusterDriver, {
  ...
  actions: {
    ...
    saveNetworking(cb) {
      ...
      if (get(this, 'vcnCreationMode') === 'Existing') {
        ...
        let loadBalancerSubnet = get(this, 'config.loadBalancerSubnet')
        let controlPlaneSubnet = get(this, 'config.controlPlaneSubnet')
        let workerNodeSubnet = get(this, 'config.workerNodeSubnet')
        let podSubnet = get(this, 'config.podSubnet')

        if (loadBalancerSubnet === controlPlaneSubnet) {
          errors.push('Load balancer subnet cannot be the same as the control plane subnet')
        }

        if (loadBalancerSubnet === workerNodeSubnet) {
          errors.push('Load balancer subnet cannot be the same as the worker node subnet')
        }

        if (loadBalancerSubnet === podSubnet) {
          errors.push('Load balancer subnet cannot be the same as the worker node subnet')
        }
      }
  ...
  validate() {
    ...
    if (get(this, 'vcnCreationMode') === 'Existing') {
      if (!get(this, 'config.controlPlaneSubnet')) {
        errors.push('Kubernetes API endpoint subnet is required')
      }
      if (!get(this, 'config.loadBalancerSubnet')) {
        errors.push('Load balancer subnet is required')
      }
      if (!get(this, 'config.workerNodeSubnet')) {
        errors.push('Worker node subnet is required')
      }
    }
    if ((get(this, 'config.cniType') === 'OCI_VCN_IP_NATIVE') && (get(this, 'vcnCreationMode') === 'Existing')) {
