...
export default Component.extend(ClusterDriver, {
  ...
  actions: {
    authenticate(cb) {
      ...
      return hash(aksRequest).then((resp) => {
        ...
        set(this, 'config.aadTenantId', data.tenantId);
  ...
  validate() {
    ...
    if (get(this, 'mode') === 'new') {
      errors = errors.concat(this.validateAad());
    }
  ...
  validateAad() {
    let errors = [];
    // check that if one SSO ID is put, then all the SSO IDs have to exists
    if (( get(this, 'config.aadClientAppId') || get(this, 'config.aadServerAppId') || get(this, 'config.aadServerAppSecret') ) && ( !get(this, 'config.aadClientAppId') || !get(this, 'config.aadServerAppId') || !get(this, 'config.aadServerAppSecret') )) {
      errors.pushObject('One of SSO field is missing');
    }
