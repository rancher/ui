...
export const LOCAL_FLEXVOLUME = 'rancher.io/localflexvolume'
...
export default Component.extend({
  ...
  didInitAttrs() {
    ...
    (get(this, 'workload.volumes')||[]).forEach((volume) => {
      ...
      } else if ( volume.flexVolume && volume.flexVolume.driver === LOCAL_FLEXVOLUME ) {
        entry = {
...
// Tmpfs (ephemeral volume -> emptyDir -> backing=memory)
  ...
  saveVolumes() {
    ...
    ary.forEach(row => {
      if (!row.mode === CUSTOM_LOG_PATH && row.volume.flexVolume.driver === LOCAL_FLEXVOLUME) {
        return;
      }
