...
export default Component.extend(NewOrEdit, ChildHook, {
  ...
  init() {
    ...
    if (!get(this, 'launchConfig.environmentFrom')) {
  ...
  willSave() {
    ...
    if (get(this, 'isSidekick')) {
      ...
      let sidekick = get(this, 'store').createRecord(def);
      ...
      let duplicate = pr.containers.find((x, idx) => {
        return idx !== lci + 1 && get(x, 'name').toLowerCase() === name;
      });
      if (duplicate) {
        errors.push(intl.t('newContainer.errors.duplicateName', { name: name, service: get(duplicate, 'displayName') }));
        set(this, 'errors', errors);
        return false;
      }
