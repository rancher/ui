...
export default Component.extend({
  ...
  nodeTemplateIdChanged: observer('nodePools.@each.{nodeTemplateId}', 'driver', function() {
    if (get(this, 'driver') === HARVESTER) {
      set(this, 'model.harvesterNodeTemplateId', get(this, 'nodePools.firstObject.nodeTemplateId'))
    }
  }),
  ...
  filteredNodeTemplates: computed('driver', 'nodePools.@each.nodeTemplateId', 'nodePools.firstObject.nodeTemplate.harvesterConfig.vmNamespace', 'nodeTemplates.@each.{driver,state}', function() {
    const driver = get(this, 'driver');
    ...
    if (driver === HARVESTER && get(this, 'nodePools.length') > 1) {
      templates.map((t) => {
        const namespace = get(this, 'nodePools.firstObject.nodeTemplate.harvesterConfig.vmNamespace')

        if ( namespace ) {
          if (get(t, 'harvesterConfig.vmNamespace') !== namespace) {
            set(t, 'disabled', true);
          } else {
      ...
      templates.map((t) => set(t, 'disabled', null))
    }
  ...
  headers: computed('driver', 'mode', 'taintCapabilites', function() {
