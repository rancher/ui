...
export default Component.extend({
  ...
  isRancherLaunchTemplate: computed('model.launchTemplate', 'originalCluster.eksStatus.managedLaunchTemplateID', function() {
    const { originalCluster, model } = this;
    const { launchTemplate } = model;
    const eksStatus = get((originalCluster ?? {}), 'eksStatus') || {};

    if (isEmpty(launchTemplate) && !isEmpty(get(eksStatus, 'managedLaunchTemplateID'))) {
      return true;
    }
