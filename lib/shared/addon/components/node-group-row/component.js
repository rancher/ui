...
export default Component.extend({
  ...
  loadTemplateVersionInfo: observer('model.launchTemplate.{id,version}', async function() {
    ...
    if (isEmpty(launchTemplate)) {
      setProperties(this.model, overrides);
      ...
      try {
        ...
        overrides = {
          imageId:      get(launchTemplateData, 'ImageId'),
          instanceType: get(launchTemplateData, 'InstanceType'),
          volumeSize:   get(launchTemplateData, 'BlockDeviceMappings.Ebs.VolumeSize'),
          sshKey:       get(launchTemplateData, 'KeyName'),
          userData:     get(launchTemplateData, 'UserData') ? atob(get(launchTemplateData, 'UserData')) : undefined,
        }
        ...
        if ( !isEmpty(get(launchTemplateData, 'TagSpecifications')) ) {
          ...
          if (!isEmpty(resourceInstanceTags) && !isEmpty(get(resourceInstanceTags, 'Tags'))) {
            set(overrides, 'resourceTags', {});

            resourceInstanceTags.Tags.forEach((tag) => set(overrides.resourceTags, get(tag, 'Key'), get(tag, 'Value')));
          }
        }
        ...
        next(() =>  set(this, 'refreshResourceInstanceTags', true) );
      } catch (err) { }
    }
  }),
