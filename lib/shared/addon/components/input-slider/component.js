...
export default Component.extend({
  ...
  valueChanged: observer('value', 'valueMin', 'valueMax', 'percent', 'elementId', function() {
    const { elementId, orig } = this
    var value = Math.max(get(this, 'valueMin'), Math.min(orig, get(this, 'valueMax')));
    ...
    if (get(this, 'elementId')) {
      $(`.${elementId}.slider-bar`).css('width', percent+'%'); // eslint-disable-line
      $(`.${elementId}.slider-handle`).css('left', percent+'%'); // eslint-disable-line
    } else {
