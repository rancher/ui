...
import { formatSi, toBaseUnit } from 'shared/utils/util';
...
export default Component.extend({
  ...
  getNodes(field) {
    return (this.get('nodes') || []).map(node => {
      ...
      if (field === 'cpu') {
        tValue *= 1000;
        uValue = node.requested[field].substr(0, node.requested[field].length - 1);
      }
      const total = toBaseUnit(tValue);
      const used = toBaseUnit(uValue);
      return {
        node,
        used,
        total,
