...
export default Component.extend(Driver, {
  ...
  workerWatch: observer('clonedModel.requestedClusterId', function() {
    let rcid = (get(this, 'clonedModel.requestedClusterId')||'');
    if (rcid.length > 0) {
      // TODO 2.0 set as default worker until others are ready
      set(get(this, 'clonedModel'), 'requestedRoles', ['worker']);
    } else {
      set(get(this, 'clonedModel'), 'requestedRoles', null);
    }
  }),
});
