...
export default Component.extend(NewOrEdit,{
  ...
  custom: computed('model.roles.[]', function() {
    ...
    return roles.filter(role => {
      return !excludes.includes(role.id)
        && get(role, 'builtin')
  ...
  customToAdd: computed('custom.@each.{active,existing}', function() {
    return get(this, 'custom').filter( role => {
      return get(role, 'active') && !get(role, 'existing');
    });
  }),

  customToRemove: computed('custom.@each.{active,existing}', function() {
    return get(this, 'custom').filter( role => {
      return get(role, 'active') === false && get(role, 'existing');
    })
  }),
