...
const ADMIN = 'cluster-owner';
const USER = 'cluster-member';
const CUSTOM = 'custom';
...
export default Component.extend(NewOrEdit,{
  ...
  init() {
    ...
    let custom  = get(this, 'model.roles').filterBy('hidden', false).filter((role) => {
      return role.get('id') !== `${get(this, 'type')}-admin` && role.get('id') !== `${get(this, 'type')}-owner` && role.get('context') === `${get(this, 'type')}`;
    }).map((role) => {
