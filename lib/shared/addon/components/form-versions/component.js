...
export default Component.extend({
  ...
  initVersions() {
    ...
    if (applyClusterTemplate || clusterTemplateCreate) {
      let overrideMatch = clusterTemplateQuestions.findBy('variable', 'rancherKubernetesEngineConfig.kubernetesVersion');

      if (overrideMatch && !isEmpty(overrideMatch.satisfies)) {
        // set(this, 'noOverrideHasSatisfies', true);
    ...
    let mappedVersions = sortVersions(out).reverse().map((v) => {
      ...
      if ((supportedVersionsRange && !satisfies(version, supportedVersionsRange) ) || (editing && initialVersion && ltr(version, coerceVersion(initialVersion)))) {
        if (ltr(version, coerceVersion(maxVersion))) {
          setProperties(out, {
            disabled: true,
