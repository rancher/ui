...
export default Mixin.create(ThrottledResize, {
  ...
  didInsertElement() {
    ...
    if (this.get('showHeader')) {
      ...
      $(this.element).find('> table > thead > .fixed-header-actions, > table > thead > .fixed-header')
        .css('width', $(this.element).find('> table')
          .outerWidth());
  ...
  buildTableWidths() {
    ...
    if ($(this.element).is(':visible')) {
      ...
      $(this.element).find('> table > thead > tr.fixed-header-placeholder > th')
        .each((idx, th) => {
      ...
      if (this.get('showHeader')) {
        ...
        $fixHdr.css({
          'width': $(this.element).find('> table')
            .width(),
        });
        if ($fixHdr.is(':visible')) {

          $(this.element).find('.search-group')
            .show(1, 'linear');
  ...
  tearDownTableWidths() {
    ...
    $(this.element).find('> table > thead > tr.fixed-header > th')
      .each((idx, td) => {
