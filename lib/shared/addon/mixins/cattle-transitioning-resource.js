...
//      url          = url.replace(/https?:\/\/[^\/]+\/?/,endpoint);
  ...
  waitForCondition: function(condition, conditionType='BackingNamespaceCreated') {
    return this._waitForTestFn(function() {
      let cMap = (this.get('conditions')||[]).findBy('type', conditionType) || null;
      let out = false;
      if (cMap) {
        out = cMap.get('status').toLowerCase() == condition; // TODO Status is a string and not a boolean
      }

      // console.log(`Wait for ConditionType: ${conditionType}: ${condition}`, this.get('conditions'));
      return out;
    }, `Wait for ConditionType: ${conditionType}: ${condition}`);
  },
