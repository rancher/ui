...
export default Mixin.create({
  ...
  processQueue() {
    ...
    while ( event ) {
      ...
      //console.log(`${this.label} Change`, type +':'+ event.data.id, clusterId, projectId);
      ...
      if ( get(this,'updateClusterStore') && clusterId && clusterStore.hasType(type) ) {
  ...
  connect() {
    ...
    var url       = ("ws://"+window.location.host + this.get('endpoint')).replace(this.get('app.projectToken'), projectId);
