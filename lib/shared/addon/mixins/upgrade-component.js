...
export default Mixin.create({
  ...
  updateStatusHelm() {
    ...
    let catalogTemplate = this.get('model.catalogTemplate');
    let upgradeVersions={};
    ...
    upgradeVersions = parseUpgradeVersions(catalogTemplate.get('versionLinks'), info.version, this.get('model'));

    if (Object.keys(upgradeVersions).length >= 1) {
      this.set('upgradeStatus', AVAILABLE);
    } else {
      this.set('upgradeStatus', NONE);
    }
