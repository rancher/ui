...
export default Mixin.create({
  ...
  getComponentInsufficient(component, type) {
    ...
    const maxLeft = get(this, `${ component }SchedulableNodes`).reduce((out, node) => {
      ...
      if (type === 'cpu') {
        left =  convertToMillis(get(node, `allocatable.${ type }`)) - convertToMillis(get(node, `requested.${ type }`));
      } else if (type === 'memory') {
        left =  parseSi(get(node, `allocatable.${ type }`)) - parseSi(get(node, `requested.${ type }`));
      }
