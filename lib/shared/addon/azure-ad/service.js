...
export default Service.extend({
    ...
    let timer = setInterval(function() {
      ...
      if (popup && popup.closed ) {
        ...
        if( !responded ) {
          ...
          cb({type: 'error', message: get(this, 'intl').t('authPage.azuread.test.authError')});
        ...
        if( !responded ) {
          ...
          cb({type: 'error', message: get(this, 'intl').t('authPage.azuread.test.popupError')});
  ...
  finishTest(config, code, cb) {
    ...
    return this.saveConfig(config, out).then(() => {
      ...
      return get(this, 'globalStore').find('principal', null, {filter: {me: true, provider: 'azuread'}}).then(( resp ) => {
