...
export default Service.extend({
  ...
  loadAllDrivers() {
    ...
    return globalStore.findAll('machinedriver').then((possible) => {
      let promises = [];

      possible.filterBy('state','active').forEach((driver) => {
        let schemaName = driver.get('name') + 'Config';
        promises.push(globalStore.find('schema', schemaName).then(() => {
          drivers.push(driver);
        }).catch(() => {
          return resolve();
        }));
  ...
  getHost(hostId) {
    ...
    return store.find('machine', hostId).then((host) => {
  ...
  getModel(params=null) {
    let promises = {
      reloadHost: this.get('clusterStore').find('schema','machine', {forceReload: true}),
      loadCustomUi: this.loadCustomUi(),
      schemas: this.get('clusterStore').find('schema'),
