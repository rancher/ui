...
export function toBaseUnit(str, increment=1024) {
  var units = ['B', 'K', 'M', 'G', 'T', 'P'];
  if (!str) {
    return;
  }
  ...
  let valStr = str.substr(0, str.length - 1);
  if (str.endsWith('i')) {
    if (str.length === 1) {
      return;
    }
    if (str) {
      unit = str[str.length - 2];
      valStr = str.substr(0, str.length - 2);
    }
  }
  if (units.indexOf(unit.toUpperCase()) === -1) {
    unit = units[0];
    valStr = str;
  }
  ...
  let exp = units.indexOf(unit.toUpperCase())
  while (exp > 0) {
    val = val * increment;
    exp--;
  }
