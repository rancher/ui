const DEFAULT_GAUGE_COLOR = '#A7C645';
const TEXT_COLOR = '#8C8F91';
const TICK_COLOR = '#2E4054'

export default function initGraph(options) {
  const { el, width, height, margin, thickness, gaugeColor, fontSize, triangleSize } = getConfig(options);
  ...
  const { valuePath, maxPath } = addArcValue(svg, width, margin, thickness, gaugeColor, options.value);
...
function addArcValue(svg, width, margin, thickness, gaugeColor, value) {
  const valuePath = addArc(svg, width, margin, thickness, gaugeColor, value);
...
function addTicks(svg, triangleSize, tooltip, width, height, margin, ticks) {
  (ticks || []).forEach(tick => {
    ...
    return svg.append('path')
      ...
      .attr('tick', 'custom')
      .attr('fill', TICK_COLOR).on("mouseover", function () {
        tooltip.transition()
          .duration(200)
          .style("opacity", .9);
        tooltip.html(`<table>
