...
export default function initGraph(options) {
  ...
  return {
    ...
    updateValue(text, live, maxValue) {
      live = live ? `${ live }%` : '';
      ...
      if (maxValue) {
        ...
        maxPath.attr('d', createArc(-135, maxValue, margin + r, Math.round(thickness / 2)));
      }

      if (text) {
        value = parseInt(text, 10);

        valuePath.attr('d', createArc(-135, multiMaxPercent(value, currentMaxValue), margin + r, Math.round(thickness / 2)));
      }
...
function repaintArc(width, margin, value, thickness, valuePath, maxPath, liveMaxPath, liveValuePath, live, currentMaxValue) {
  ...
  maxPath.attr('d', createArc(-135, currentMaxValue, width / 2, Math.round(thickness / 2)))
    .attr('transform', `translate(${  margin + r
    },${  margin + r  }), scale(1, 1)`);
  console.log(multiMaxPercent(value, currentMaxValue), 'multiMaxPercent(value, currentMaxValue)')
  valuePath.attr('d', createArc(-135, multiMaxPercent(value, currentMaxValue), width / 2, Math.round(thickness / 2)))
    .attr('transform', `translate(${  margin + r
    },${  margin + r  }), scale(1, 1)`);
}
...
function addArcValue(svg, width, margin, thickness, value, options, live) {
  ...
  const liveValuePath = addArc(svg, width, margin, thickness, 'gauge-value-path', live > 100 ? 100 : live);
  const maxPath = addArc(svg, width, 0, Math.round(thickness / 2), 'gauge-max-path', options.maxValue);
  const valuePath = addArc(svg, width, 0, Math.round(thickness / 2), 'gauge-live-value-path', value > 100 ? 100 : value);
...
function addTicks(svg, tooltip, width, height, margin, ticks, currentValue, thickness, liveTicks = [], live, currentMaxValue) {
  ...
  (ticks || []).forEach((tick) => {
    ...
    const point = valueToPoint(width, height, 0, multiMaxPercent(value, currentMaxValue), thickness / 2);
  ...
  if (liveTicks && liveTicks.length > 1 && (liveMax - liveMin >= 1)) {
    const point = valueToPoint(width, height, margin, live, thickness);
    ...
    const rangePath = addArc(svg, width, margin, thickness, 'gauge-tick-path', liveMax, liveMin, 2);

    rangePath.attr('tick', 'custom');
  }

  if (ticks && ticks.length > 1) {
    const point = valueToPoint(width, height, 0, multiMaxPercent(currentValue, currentMaxValue), thickness / 2);
    ...
    const rangePath = addArc(svg, width, 0, thickness / 2, 'gauge-tick-path', multiMaxPercent(max, currentMaxValue), multiMaxPercent(min, currentMaxValue), 2);
