...
function addArcValue(svg, width, margin, thickness, value, options, live) {
  ...
  const valuePath = addArc(svg, width, OUT_RING_MARGIN, Math.round(thickness / 2), 'gauge-live-value-path', value > 100 ? 100 : value);
...
function addTicks(svg, tooltip, width, height, margin, ticks, currentValue, thickness, liveTicks = [], live, currentMaxValue) {
  ...
  if (ticks && ticks.length > 1) {
    const point = valueToPoint(width, height, OUT_RING_MARGIN, multiMaxPercent(currentValue, currentMaxValue), thickness / 2);
    ...
    const rangePath = addArc(svg, width, OUT_RING_MARGIN, thickness / 2, 'gauge-tick-path', multiMaxPercent(max, currentMaxValue), multiMaxPercent(min, currentMaxValue), 2);
