export default function initGraph(options) {
  ...
  const rectangle = svg.append('rect').attr('x', r)
    .attr('y', margin)
    .attr('width', width)
    .attr('height', height - margin * 2)
    .attr('class', `${ healthyStatus } background`);

  const circle = svg.append('circle').attr('cx', r)
    .attr('cy', r)
    .attr('r', r)
...
function fit(svg, el, rectangle, circle, text, icon) {
  ...
  rectangle.attr('x', r).attr('y', margin)
    .attr('width', width)
    .attr('height', height - margin * 2);
  circle.attr('cx', r).attr('cy', r)
    .attr('r', r);
  text.attr('x', 3 * r).attr('y', height / 2)
    .attr('dy', fontSize / 2)
    .style('font-size', `${ fontSize  }px`);
  icon.attr('x', 0.5 * r).attr('y', 0.5 * r)
    .attr('width', r)
    .attr('height', r);
