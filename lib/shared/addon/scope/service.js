...
export default Service.extend({
  ...
  getAllClusters: function() {
    ...
    return this.get('userStore').find('cluster', null, opt).then(() => {
      return this.get('userStore').all('cluster');
    });
  },
  ...
  selectDefaultProject: function(desired) {
    ...
    function select(project, overwriteDefault) {
      if (project.get('clusterId')) {
        self.set('currentCluster', self.get('userStore').getById('cluster', project.get('clusterId'))||{});
      }
      if ( project )
      {
  ...
  setCurrentCluster: function(cluster) {
    ...
    this.set('currentPageScope', 'custer');
    // @@TODO@@ - 11-10-17 - need a new store for clusters
    // if ( cluster ) {
    //   this.set('store.baseUrl', `${this.get('app.apiEndpoint')}/clusters/${cluster.get('id')}`);
    // } else {
  ...
  setCurrent: function(project) {
    ...
    this.set('currentPageScope', 'project');
    if ( project ) {
  ...
  _activeClusterFromId: function(clusterId) {
    return new EmberPromise((resolve, reject) => {
      ...
      this.get('userStore').find('cluster', clusterId, {url: `clusters/${encodeURIComponent(clusterId)}`}).then((cluster) => {
