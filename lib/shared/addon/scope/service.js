...
export default Service.extend({
  ...
  getAll: function(moreOpt={}) {
    ...
    return this.get('authzStore').find('project', null, opt).then(() => {
      return this.get('authzStore').all('project');
    });
  },
  ...
  _activeProjectFromId: function(projectId) {
    return new EmberPromise((resolve, reject) => {
      ...
      this.get('authzStore').find('project', projectId, {url: 'projects/'+encodeURIComponent(projectId)}).then((project) => {
